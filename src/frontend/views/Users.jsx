...
const Users = () => {
  ...
  const userId = typeof window !== 'undefined' ? (localStorage.getItem('user_id') || 'system') : 'system';
  const currentUserRole = typeof window !== 'undefined' ? (localStorage.getItem('role') || 'Owner') : 'Owner'; // Mocking role for now
  ...
  const fetchUsers = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${(process.env.NEXT_PUBLIC_API_URL || '')}/users?company_name=${company}`);
      if (res.ok) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isEdit) {
        url = `${(process.env.NEXT_PUBLIC_API_URL || '')}/users/${selectedUser.id}`;
        method = 'PUT';
      } else {
