...
import {
  Box, CssBaseline, Typography, Button, TextField,
  Switch, IconButton, Tooltip, Paper, Dialog, DialogTitle,
...
// ── Status Row (single work status inside a stage card) ──────────────────────
  ...
  return (
    <Box
      ...
      sx={{
        ...
        px: 1.5,
        py: 0.65,
        borderTop: wsIdx === 0 ? 'none' : '1px solid',
        borderColor: 'divider',
        borderLeft: `3px solid ${item.is_default ? stageColor : 'transparent'}`,
        bgcolor: item.is_default
          ? stageColor + '0c'
          : hovered ? 'action.hover' : 'transparent',
    ...
    >
      ...
      <Tooltip title={item.is_default ? 'Default status' : 'Set as default'} placement="left">
        <IconButton size="small" onClick={onDefault} sx={{ p: 0.3, color: item.is_default ? stageColor : 'transparent', '&:hover': { color: stageColor } }}>
          {item.is_default
            ...
            : <StarOutlineRoundedIcon sx={{ fontSize: 14 }} />
          }
        </IconButton>
      </Tooltip>
      ...
      <InlineEdit
        ...
        inputWidth={100}
        sx={{ flexGrow: 1, color: item.is_default ? stageColor : 'text.primary' }}
      />
...
const WorkflowSettings = () => {
  ...
  const handleDeleteStage = (id, name) => {
    setTaskWorkflow(p => ({ ...p, stages: p.stages.filter(s => s.id !== id) }));
    setTaskWorkStatus(p => { const m = { ...p }; delete m[name]; return m; });
    toast.success(`"${name}" deleted`);
  };
  ...
  const handleDeleteWS = (stage, sid, name) => {
    setTaskWorkStatus(p => {
      const filtered = (p[stage] || []).filter(i => i.id !== sid);
      if (filtered.length > 0 && !filtered.some(i => i.is_default)) filtered[0].is_default = true;
      return { ...p, [stage]: filtered };
    });
    toast.success(`"${name}" deleted`);
  };
  ...
  return (
    <>
      ...
      <Box sx={{ display: 'flex', height: '100vh', overflow: 'hidden', bgcolor: 'background.default' }}>
        <Sidebar company={company} activeMenu="Workflow" />
        <Box sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden' }}>
          ...
          <Box sx={{
            ...
            <Box sx={{ flexGrow: 1 }}>
              <Typography sx={{ fontWeight: 700, fontSize: '1rem', lineHeight: 1.2 }}>Workflow Settings</Typography>
              <Typography variant="caption" color="text.secondary">
                Configure project hierarchy and task workflow stages for your team.
              </Typography>
            </Box>
            <Button size="small" variant="text" sx={{ color: 'text.secondary', fontWeight: 500 }} onClick={() => router.push(`/${company}/dashboard`)}>
              Cancel
            </Button>
            <Button size="small" variant="outlined" startIcon={<RestartAltIcon />} onClick={handleReset} sx={{ borderRadius: 2 }}>
              Reset
            </Button>
            <Button
          ...
          <Box sx={{ flexGrow: 1, overflowY: 'auto', p: 3, display: 'flex', flexDirection: 'column', gap: 3 }}>
            <Box sx={{ maxWidth: 1300, mx: 'auto', width: '100%', display: 'flex', flexDirection: 'column', gap: 3 }}>

              {/* ══════════════════════════════════════════════════════
                  SECTION A — PROJECT FLOW
                  ══════════════════════════════════════════════════════ */}
                ...
                <Box sx={{ px: 3, py: 2.5, display: 'flex', flexDirection: 'column', gap: 0 }}>
                  ...
                  <Box sx={{ ml: 2.1, pl: 2.6, borderLeft: '2px dashed', borderColor: 'divider', my: 0.5, py: 0.5 }}>
                    ...
                    <Box sx={{
                      ...
                      border: '1px solid', borderColor: storyLevel.enabled ? '#7c3aed44' : 'divider',
                      bgcolor: (t) => storyLevel.enabled
                        ? (t.palette.mode === 'dark' ? '#7c3aed12' : '#7c3aed08')
                    ...
                    <Box sx={{ ml: 1.9, pl: 2.4, borderLeft: '2px dashed', borderColor: 'divider', py: 0.5, display: 'flex', flexDirection: 'column', gap: 0.8 }}>
                      ...
                      {taskTypes.map((t, idx) => (
                        <Box
                          ...
                          sx={{
                            ...
                            px: 1.5, py: 1,
                            borderRadius: 1.5,
                            border: '1px solid', borderColor: t.enabled ? '#05996922' : 'divider',
                            bgcolor: 'background.paper',
                            '&:hover .type-actions': { opacity: 1 },
                          }}
                        >
                          ...
                          <Chip
                            label={t.is_custom ? 'Custom' : 'Built-in'}
                            size="small"
                          ...
                          <Box className="type-actions" sx={{ ml: 'auto', display: 'flex', alignItems: 'center', gap: 0.5, opacity: 0, transition: 'opacity 0.15s' }}>
                            ...
                            <IconButton size="small" disabled={idx === 0} onClick={() => moveTaskType(idx, 'up')} sx={{ p: 0.3 }}>
                              <KeyboardArrowUpIcon sx={{ fontSize: 14 }} />
                            </IconButton>
                            <IconButton size="small" disabled={idx === taskTypes.length - 1} onClick={() => moveTaskType(idx, 'down')} sx={{ p: 0.3 }}>
                      ...
                      <Button
                        ...
                        variant="outlined"
                        color="success"
                        sx={{ alignSelf: 'flex-start', borderRadius: 1.5, fontSize: '0.75rem', borderStyle: 'dashed', mt: 0.5 }}
                      >
                        Add Task Type
                      </Button>
              ...
              <Paper elevation={0} sx={{ border: '1px solid', borderColor: 'divider', borderRadius: 3, overflow: 'hidden' }}>
                ...
                <Box sx={{
                  ...
                  <Button
                    size="small" variant="contained"
                    startIcon={<AddIcon />}
                    onClick={() => setNewStageModal(true)}
                    sx={{ borderRadius: 2, fontSize: '0.75rem', fontWeight: 600, px: 2 }}
                  >
                    Add Stage
                  </Button>
                </Box>
                  ...
                  <Box sx={{ display: 'flex', gap: 2, minWidth: 'max-content', pb: 0.5 }}>
                      ...
                      return (
                        <Paper
                          ...
                          sx={{
                            ...
                            '&:hover': { boxShadow: `0 2px 16px ${stageColor}22` },
                            '&:hover .stage-header-actions': { opacity: 1 },
                          }}
                        >
                          {/* Card Header */}
                          <Box sx={{
                            ...
                            bgcolor: (t) => t.palette.mode === 'dark' ? stageColor + '14' : stageColor + '09',
                          }}>
                            {/* Stage icon + name row */}
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1.5 }}>
                              <Tooltip title="Change icon">
                                <IconButton
                                  size="small"
                                  onClick={() => { setEditingStageId(stage.id); setIconPickerOpen(true); }}
                                  sx={{ p: 0.6, color: stageColor, bgcolor: stageColor + '18', borderRadius: 1.5, '&:hover': { bgcolor: stageColor + '28' } }}
                                >
                              ...
                              <InlineEdit
                                value={stage.name}
                                onChange={n => updateStageName(stage.id, n)}
                                sx={{ color: stageColor }}
                                inputWidth={100}
                              />
                            </Box>
                            ...
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                              ...
                              <Tooltip title="Move left"><span>
                                <IconButton size="small" disabled={sIdx === 0} onClick={() => moveStage(sIdx, 'left')} sx={{ p: 0.3 }}>
                                  <ChevronLeftIcon sx={{ fontSize: 14 }} />
                                </IconButton>
                              </span></Tooltip>
                              <Tooltip title="Move right"><span>
                                <IconButton size="small" disabled={sIdx === stages.length - 1} onClick={() => moveStage(sIdx, 'right')} sx={{ p: 0.3 }}>
                          ...
                          <Box sx={{ px: 1.5, py: 1, borderTop: '1px solid', borderColor: 'divider' }}>
                            <Button
                              ...
                              onClick={() => { setTargetStageName(stage.name); setNewStatusModal(true); }}
                              sx={{
                                fontSize: '0.72rem', fontWeight: 500,
                                color: stageColor, borderRadius: 1.5,
                                justifyContent: 'flex-start',
                                '&:hover': { bgcolor: stageColor + '12' }
                              }}
                            >
                              Add work status
                            </Button>
                    ...
                    <Box
                      ...
                      sx={{
                        ...
                        '&:hover': { borderColor: 'primary.main', color: 'primary.main', bgcolor: 'primary.light', opacity: 0.08 }
                      }}
                    >
      ...
      <Dialog open={newTypeModal} onClose={() => setNewTypeModal(false)} maxWidth="xs" fullWidth slotProps={{ paper: { sx: { borderRadius: 3 } } }}>
        ...
        <DialogContent>
          <TextField
            autoFocus fullWidth size="small" label="Type Name"
            placeholder="e.g. Feature, Incident, Sub-task"
      ...
      <Dialog open={newStageModal} onClose={() => setNewStageModal(false)} maxWidth="xs" fullWidth slotProps={{ paper: { sx: { borderRadius: 3 } } }}>
        ...
        <DialogContent sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          <TextField
            autoFocus fullWidth size="small" label="Stage Name"
            placeholder="e.g. QA, UAT, Code Review"
                ...
                <Tooltip key={c} title={c}>
                  <Box
                    ...
                    sx={{
                      ...
                      border: newStageColor === c ? '3px solid #000' : '2px solid transparent',
      ...
      <Dialog open={newStatusModal} onClose={() => setNewStatusModal(false)} maxWidth="xs" fullWidth slotProps={{ paper: { sx: { borderRadius: 3 } } }}>
        ...
        <DialogContent>
          <TextField
            autoFocus fullWidth size="small" label="Status Name"
            placeholder="e.g. In Review, Approved, Escalated"
            value={newStatusName} onChange={e => setNewStatusName(e.target.value)}
          ...
          <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1.5, justifyContent: 'center', pt: 1 }}>
            {Object.entries(ICON_MAP).map(([name, Comp]) => (
              <Tooltip key={name} title={name}>
                <IconButton
                  onClick={() => updateStageIcon(editingStageId, name)}
                  sx={{ p: 1.5, border: '1px solid', borderColor: 'divider', borderRadius: 2, '&:hover': { bgcolor: 'primary.light', opacity: 0.7 } }}
                >
                  <Comp sx={{ fontSize: 22 }} />
                </IconButton>
              </Tooltip>
