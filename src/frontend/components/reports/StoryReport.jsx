...
const StoryReport = ({ projects = [], storiesByProject = {}, tasksByStory = {}, users = [] }) => {
  ...
  const handleExportCSV = () => {
    const headers = [
      ...
      'Todo', 'Develop', 'Code Review', 'Testing', 'Deploy', 'Done', 'Progress', 'Est. Hours', 'Total Hours',
  ...
  const columns = [
    { field: 'name', headerName: 'Story Name', flex: 1.5, minWidth: 130, renderCell: (params) => <strong style={{ color: '#8b5cf6', fontSize: '0.78rem' }}>{params.value}</strong> },
    { field: 'projectName', headerName: 'Project', flex: 1, minWidth: 100 },
    ...
    { field: 'done', headerName: 'Done', width: 60, type: 'number', headerAlign: 'center', align: 'center' },
    {
      ...
      renderCell: (params) => (
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, width: '100%', height: '100%' }}>
          <Box sx={{ flexGrow: 1 }}>
            <LinearProgress variant="determinate" value={params.value} sx={{ height: 5, borderRadius: 3, bgcolor: 'rgba(255,255,255,0.05)', '& .MuiLinearProgress-bar': { bgcolor: '#8b5cf6' } }} />
          </Box>
          <Typography variant="caption" fontWeight="bold" fontSize="0.7rem">{params.value}%</Typography>
        </Box>
      )
    },
    { field: 'estimateHours', headerName: 'Est.Hrs', width: 70, type: 'number', headerAlign: 'center', align: 'center', renderCell: (params) => `${params.value}h` },
    { field: 'reporter', headerName: 'Reporter', flex: 1, minWidth: 90 },
    { field: 'assignee', headerName: 'Assignee', flex: 1, minWidth: 90 },
    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
      ...
      <Card sx={{ border: '1px solid rgba(255,255,255,0.06)', bgcolor: 'background.paper', borderRadius: 3 }}>
        <CardContent sx={{ p: '10px' }}>
          <Typography variant="subtitle2" fontWeight="700" sx={{ mb: 1.5 }}>Story Filter Config</Typography>
      ...
      <Card sx={{ border: '1px solid rgba(255,255,255,0.06)', bgcolor: 'background.paper', borderRadius: 3 }}>
        <Box sx={{ px: 2, py: 1, display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid rgba(255,255,255,0.06)', flexWrap: 'wrap', gap: 1 }}>
          <Typography variant="subtitle2" fontWeight="700">Story Deliverables Matrix</Typography>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 1 }}>
              ...
              <Box sx={{ bgcolor: 'rgba(255,255,255,0.02)', p: 2, borderRadius: 2 }}>
                ...
                <Grid container spacing={2}>
                  ...
                  <Grid xs={6} sm={4}>
                    <Typography variant="caption" color="text.secondary">Todo</Typography>
                    <Typography variant="h6" fontWeight="bold">{selectedRow.todo}</Typography>
                  </Grid>
                  <Grid xs={6} sm={4}>
                    <Typography variant="caption" color="text.secondary">Developing</Typography>
                    <Typography variant="h6" fontWeight="bold">{selectedRow.inProgress}</Typography>
                  </Grid>
                  <Grid xs={6} sm={4}>
                    <Typography variant="caption" color="text.secondary">Code Review</Typography>
